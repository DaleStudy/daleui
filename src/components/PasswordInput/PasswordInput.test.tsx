...
const defaultPasswordPlaceholder = "패스워드를 입력해주세요.";

function getPasswordField() {
  return screen.getByPlaceholderText(defaultPasswordPlaceholder);
}
...
test("helperText가 빈 문자열이면 도움말을 렌더하지 않고 aria-describedby를 두지 않는다", () => {
  render(<PasswordInput helperText="" />);
  const input = getPasswordField();
...
test("aria-describedby prop과 helperText를 함께 쓰면 두 id를 공백으로 이어 붙인다", () => {
  render(
    <PasswordInput aria-describedby="extra-desc" helperText="힌트입니다." />,
  );
  const input = getPasswordField();
...
test("aria-describedby prop만 있고 helperText가 빈 문자열이면 해당 id만 aria-describedby에 넣는다", () => {
  render(<PasswordInput aria-describedby="only-extra" helperText="" />);
  const input = getPasswordField();
...
test("errorMessage와 helperText가 모두 있으면 errorMessage를 표시한다", () => {
  render(
    <PasswordInput errorMessage="오류입니다." helperText="도움말입니다." />,
  );
