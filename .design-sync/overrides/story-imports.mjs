// FORK of .ds-sync/lib/story-imports.mjs for daleui (declared in cfg.libOverrides).
// Why: Panda CSS emits styled-system/* as index.mjs-only directories
// (styled-system/css/index.mjs, styled-system/patterns/index.mjs). Stories
// import them relatively (`../../../styled-system/css`), and esbuild's default
// resolveExtensions (.tsx,.ts,.jsx,.js,.css,.json) never try `.mjs`, so every
// preview compile fails with "Could not resolve". The only change is the
// `.mjs` / `index.mjs` retry in policyRedirect below (search: FORK).
  ...
  const policyRedirect = {
    ...
    setup(b) {
      b.onResolve({ filter: /.*/ }, async (a) => {
        ...
        let r = await b.resolve(a.path, {
          kind: a.kind, resolveDir: a.resolveDir, importer: a.importer,
          pluginData: 'ds-resolving',
        });
        // FORK: relative import that esbuild couldn't resolve - retry the
        // `.mjs` file / `index.mjs` directory forms Panda's styled-system uses.
        if ((r.errors.length > 0 || !r.path) && /^\.\.?\//.test(a.path) && a.resolveDir) {
          const base = resolve(a.resolveDir, a.path);
          const mjs = [base + '.mjs', join(base, 'index.mjs')].find((f) => existsSync(f));
          if (mjs) r = { errors: [], warnings: [], path: mjs, namespace: 'file' };
        }
