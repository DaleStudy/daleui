// FORK of .ds-sync/lib/dts.mjs for daleui (declared in cfg.libOverrides).
// Why: jsdocFor() sanitizes the component description with a non-unicode
// regex whose `\w` matches ASCII only, so daleui's Korean JSDoc collapses to
// bare punctuation in the README component index. The only change is the
// `u`-flag regex in jsdocFor (search: FORK). Retire this fork once the
// bundled skill preserves non-Latin letters.
...
export function jsdocFor(name, ctx) {
  ...
  return doc.split('\n').find((l) => l.trim() && !l.trim().startsWith('@'))
    ?.trim().replace(/\s+/g, ' ').replace(/[^\p{L}\p{N}_\s.,()'/:+-]/gu, '') /* FORK: keep non-Latin letters */.slice(0, 140) ?? '';
}
